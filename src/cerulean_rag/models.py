...
class DocumentMeta(BaseModel):
    """Metadata for one corpus document. The manifest is the source of truth; the
    PDF header is only a cross-check. superseded_by and is_current are derived
    from the supersedes chain, never read from a file."""
    ...
    def effective_date_int(self) -> int:
        """yyyymmdd form, for numeric comparison in Chroma filters."""
...
class LoadedDocument(BaseModel):
    """Metadata plus cleaned page texts. Running headers are stripped and page 1's
    metadata block is removed, since it is carried in meta."""
...
class Chunk(BaseModel):
    """One logical section, or one FAQ question, of a document.

    text is what is displayed and cited; text_for_embedding is the same text
    behind a context header, so the vector and the keyword index both carry
    document identity, dates and status."""
    ...
    def to_metadata(self) -> dict[str, str | int | float | bool]:
        """Flat scalars only; Chroma rejects None and nested values."""
        import json
...
class AnswerSchema(BaseModel):
    """The single JSON object the model must return.

    decision is a validated string rather than a Literal so that a small model's
    casing and hyphenation variants ("Insufficient-Evidence") are normalised
...
class RetrievedChunk(BaseModel):
    """A chunk in the fused result set, with the evidence for its rank."""
...
class RetrievalBundle(BaseModel):
    """Everything retrieval hands to prompt assembly; inspectable without an LLM."""
...
class AnswerResult(BaseModel):
    """The verified answer plus everything needed to audit how it was produced."""
