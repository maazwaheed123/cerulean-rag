...
_WARNING_PLAIN: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"^citation to (?P<d>\S+): the quote does not appear in the retrieved text$"),
     "The quote shown for {d} could not be found in the retrieved text - treat it as unverified."),
    ...
    (re.compile(r"^conflict '(?P<t>.*?)' quotes (?P<d>\S+) but the answer does not cite it$"),
     "The reported conflict about “{t}” quotes {d}, but {d} is not in the citations - that side is unbacked."),
