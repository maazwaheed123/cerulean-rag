...
4. Combine across documents when a question needs it. Show calculations step by step, citing the source of each input, and state any assumption you had to make. For a calculation over a date range, list each calendar month in the range with the number of days served in it, apply the stated rule to each month, then add up the result.
